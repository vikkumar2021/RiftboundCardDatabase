...
export class DataIngestionService {
  ...
  private normalizeCard(raw: RiftboundRawCard) {
    ...
    return {
      card: {
        riotId: String(raw.id),
        publicCode: String(raw.publicCode || `${setId}-${raw.collectorNumber}`),
        ...
        energy: raw.energy?.value?.id != null ? String(raw.energy.value.id) : null,
        might: raw.might?.value?.id != null ? String(raw.might.value.id) : null,
        power: raw.power?.value?.id != null ? String(raw.power.value.id) : null,
        tags: tags,
        rulesHtml: rulesHtml,
        rulesText: rulesText,
